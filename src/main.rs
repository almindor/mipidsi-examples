...
type ConcreteDisplay = Display<SPIInterfaceNoCS<SpiExclusiveDevice<QSPI1, (Pin3<IOF0<NoInvert>>, (), Pin5<IOF0<NoInvert>>)>, Pin2<Output<Regular<NoInvert>>>>, Pin1<Output<Regular<NoInvert>>>, ST7789>;

fn init() -> ConcreteDisplay {
    ...
    sprintln!("basic init done");

    // let _backlight = pins.pin10.into_output();
    let rst = pin!(pins, dig9).into_output(); // reset pin
    // let _cs = pin!(pins, spi0_cs0).into_output(); // keep low while drivign display
    let dc = pin!(pins, dig10).into_output(); // data/clock switch
